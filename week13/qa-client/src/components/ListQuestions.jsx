...
function ListQuestions(props) {
    ...
    const questions = isRoot
        ? props.questions
        : props.questions.filter(q => q.email === props.user.email);
    ...
    return (
        <Container fluid className="mt-3">
            ...
            {props.loggedIn ?  <h1>{isRoot ? "List of All Questions" : "List of Asked Questions"}</h1> : <h1>{"List of All Questions"}</h1>}
            <div>
                {questions.map((q) => {
